...
const sendgridClientPromise = initializeSendGridClient();
...
export async function sendEmail(to: string, subject: string, html: string, fromName: string = "colormind"): Promise<void> {
  // 1. Aguarda a promessa de inicialização ser resolvida
  const client = await sendgridClientPromise;

  // 2. Verifica se o cliente foi inicializado com sucesso
  if (!client) {
    logger.error("Tentativa de enviar e-mail, mas o cliente Sendfirebase deploy --only functionsGrid não está inicializado.");
    throw new HttpsError("internal", "Serviço de e-mail não está disponível.");
  }

  // 3. Monta a requisição (a lógica interna permanece a mesma)
  const request = {
  ...
  try {
    // 4. Envia a requisição usando o cliente já configurado
  ...
  switch (type) {
    ...
    case 'healthConditions':
      prompt = `${persona} ${baseInstruction}
        ...
        - Lista existente: ["Diabetes"], Entrada do usuário: "tenho diabete e pressão alta", Resultado esperado: {"items": ["Hipertensão"], "reason": "processed"}
    ...
    case 'dietaryRestrictions':
      prompt = `${persona} ${baseInstruction}
        ...
        3. Validação de Categoria: A entrada DEVE ser um estilo de vida (vegano), uma restrição conhecida (sem glúten), ou um alimento/grupo alimentar a ser evitado.
    ...
    case 'currentMedications':
      prompt = `${persona} ${baseInstruction}
        ...
        - Lista existente: [], Entrada do usuário: "chá de camomila", Resultado esperado: {"items": [], "reason": "irrelevant"}
        Entrada do usuário para analisar: "${text}"`;
      break;
    default:
